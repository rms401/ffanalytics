...
def rts_draft(metric: str = "adp") -> pd.DataFrame:
    ...
    frame = _cached(f"adp_rts_{metric}", build)
    return pd.DataFrame({
...
def mfl_draft(metric: str = "adp", n_teams: int = 12) -> pd.DataFrame:
    ...
    return _cached(f"adp_mfl_{metric}", build)
...
def ffc_draft(metric: str = "adp", n_teams: int = 12,
    ...
    return _cached(f"adp_ffc_{scoring}_{n_teams}", build)
