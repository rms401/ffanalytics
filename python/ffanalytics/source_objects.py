"""Per-source column maps -- the schema definition for every scraper.

Transcribed from ``R/source_objects.R``.  Each maps a site's raw column header
(or stat id) onto the package's canonical snake_case stat vocabulary.

These are :class:`NamedVec`, not dicts, because several of them repeat a name
and R's lookup returns the *first* match.  ``espn_columns`` is the clearest
case: stat id ``109`` means ``dst_tackles`` for a team and ``idp_solo`` for an
individual defender, and ``scrape_espn`` picks between them by filtering the
vector (``R/source_scrapes.R:1318,1325``) rather than by key.
"""
...
# Populated from MyFantasyLeague when the package is first used; see
# ffanalytics.player_data.  Mirrors the `player_table = NULL` placeholder in
# R/source_objects.R:6.
player_table = None

cbs_columns = NamedVec([
    ('Player', 'player'),
    ('Team', 'team'),
...
# duplicate names (R returns the first match): ['Ret TD']
nfl_columns = NamedVec([
    ('Player', 'player'),
    ('id', 'src_id'),
    ('name', 'player'),
    ('teamabbr', 'team'),
...
# duplicate names (R returns the first match): ['>= 100yds', '>= 50yds', 'Att', 'DefTD', 'Fum']
fantasysharks_columns = NamedVec([
    ('Player ID', 'id'),
    ('Player Name', 'player'),
    ('Position', 'pos'),
    ('Team', 'team'),
...
numberfire_idp_columns = NamedVec([
    ('id', 'id'),
    ('numfire_id', 'src_id'),
    ('Player', 'player'),
    ('position', 'pos'),
...
walterfootball_columns = NamedVec([
    ('Player', 'player'),
    ('Team', 'team'),
    ('Bye', 'bye'),
    ('position', 'pos'),
...
# duplicate names (R returns the first match): ['FR', 'INT', 'Sack']
fftoday_columns = NamedVec([
...
# duplicate names (R returns the first match): ['Defense FF', 'Defense FR', 'Defense INT', 'Defense Sack', 'Defense TD']
fleaflicker_columns = NamedVec([
    ('Player Name', 'player'),
    ('Opp', 'opp'),
    ('FPts', 'curr_site_pts'),
...
nflreadr_offense_cols = NamedVec([
    ('player_id', 'gsis_id'),
...
# duplicate names (R returns the first match): ['106', '109', '95', '96', '99']
espn_columns = NamedVec([
    ('0', 'pass_att'),
    ('1', 'pass_comp'),
    ('3', 'pass_yds'),
    ('4', 'pass_tds'),
